...
git-std auto-detects `Cargo.toml` (supporting workspace manifests) and reads/writes only the `version` field during `bump`.
...
**Version file resolution:** git-std auto-detects `Cargo.toml`. For workspace manifests, it finds the binary crate member. Reads and writes the `version` field only.
...
Without `--full`, generates an incremental changelog (unreleased commits since the last tag) and prepends to the output file.
