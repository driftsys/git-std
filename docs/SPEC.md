...
4. Apply bump rules inferred from `scheme`: if any commit has a `BREAKING CHANGE` footer or `!` after the type → major. Otherwise, the highest bump wins (`minor` > `patch`).
...
**Advanced:** if `cliff.toml` exists alongside `.git-std.toml`, it's passed to `git_cliff_core` for template customization. `.git-std.toml` `[changelog.sections]` takes precedence for section grouping.
