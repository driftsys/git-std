...
pub enum Command {
    ...
    /// files only. Pre-commit commands receive matching staged files in $@.
    /// Other hooks receive Git's original arguments; for pre-push, $1 is the
    /// remote name, $2 is the remote URL, and ref updates are provided on stdin:
    ///
    ///   ~ cargo fmt -- $@   *.rs
    ///   ! cargo clippy
    ///
